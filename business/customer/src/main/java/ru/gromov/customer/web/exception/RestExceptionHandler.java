...
import ru.gromov.customer.exception.NotFoundException;
...
public class RestExceptionHandler implements ProblemHandling {
    ...
    public ResponseEntity<Problem> process(@Nullable ResponseEntity<Problem> entity,
        ...
        ProblemBuilder builder = Problem.builder()
                .withType(Problem.DEFAULT_TYPE.equals(problem.getType()) ?
                        ExceptionConstants.DEFAULT_TYPE : problem.getType())
    ...
    public ResponseEntity<Problem> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        List<ExceptionField> fieldErrors = result.getFieldErrors().stream()
                .map(f -> new ExceptionField(f.getObjectName(), f.getField(), f.getCode()))
    ...
    @ExceptionHandler
    public ResponseEntity<Problem> handleNoSuchElementException(NotFoundException ex, NativeWebRequest request) {
        Problem problem = Problem.builder()
                .withStatus(Status.NOT_FOUND)
                .with(MESSAGE_KEY, ExceptionConstants.ENTITY_NOT_FOUND_TYPE)
                .build();
        return create(ex, problem, request);
    }
