...
public enum SMSResponseStatus {
    ...
    ;
    private final static Map<Integer, SMSResponseStatus> statusMap =
            new HashMap<Integer, SMSResponseStatus>(SMSResponseStatus.values().length, 1f);

    static {
        for (SMSResponseStatus status : values()) {
    ...
    SMSResponseStatus(int code) {
        this.code = code;
    }

    public static SMSResponseStatus forValue(int code) {
